# ADR-028: One naming namespace, anchored in `the design doc`; private identifiers CI-gated

- **Status**: Accepted
- **Date**: 2026-05-31
- **Deciders**: project maintainer
- **Related**: ADR-021 (schema layout), ADR-022 (error handling), ADR-027 (unknown-field capture); `the design doc` §15.8 (engineering disciplines).
...
repository, and it is the namespace `the design doc` and the ADR series already
establish.** Specifically:

- **Tools** by name and number from `the design doc` §3 — *Tool 1 (recompile aggregator)*, *Tool 2 (compile-diff)*, …
...
- **Option C — Keep section-level planning private; gate private identifiers
  out of public artifacts; reuse `the design doc`'s existing namespace for public
  reference.** Chosen.
