...
- **Related**: ADR-024 (FX-graph node contract — the ordered `inputs` this policy interprets); `design.md` §8.2.5 (WL-signature diff); D4 (the diff is a pure function with no torch dependency).
