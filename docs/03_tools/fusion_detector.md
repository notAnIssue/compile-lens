...
Each opportunity names a suggested kernel as a string (e.g. `fold per-row 1/rms into the second GEMM epilogue`).
...
- Suggested kernel: fold per-row 1/rms into the second GEMM epilogue
- Confidence: high
```
