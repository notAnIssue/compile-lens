...
| `nonshape_guards.log` | < 1 KB | A **non-shape** guard recompile (`n == 1`, a python-int specialization). Unlike size/dtype/stride mismatches it carries **no** `expected/actual` value, and torch appends a ` # <source> # <file>:<line>` comment the parser must strip. Also includes the `- User stack trace:` continuation block that follows a failure. This is the case the shape-only fixtures hid — the reason `failed_guard` values are optional. |
