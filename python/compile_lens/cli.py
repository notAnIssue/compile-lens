...
This is a thin dispatcher (the design doc P7): the real work lives in the library
...
def _add_collect_args(sp: argparse.ArgumentParser) -> None:
    ...
    mirroring the Rust ``cl collect`` surface (the design doc ADR-006: the Python ``cl`` is the
...
def _add_compile_lint_args(sp: argparse.ArgumentParser) -> None:
    ...
    (the design doc ADR-006; the unified single-command front-end is the Phase 7 hero)."""
